...
Layer3::get_client_addr ()
{
    ...
    for (int i = 1; i <= client_addrs_len; i++)
      {
        ...
        if (! hasAddress (a))
          {
            TRACEPRINTF (t, 3, this, "Allocate %d.%d.%d",
                         (a >> 12) & 0xf, (a >> 8) & 0xf, a & 0xff);
  ...
  TRACEPRINTF (t, 3, this, "Allocate: falling back to %d.%d.%d",
               (defaultAddr >> 12) & 0xf, (defaultAddr >> 8) & 0xf, defaultAddr & 0xff);
  return 0;
}
